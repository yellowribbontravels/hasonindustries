    ...
    <section className="py-24 bg-transparent border-t border-neutral-200/60 relative z-10">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-['Bebas_Neue'] tracking-widest text-[#09090B]">Operational <span className="text-[#10B981]">Command</span></h2>
          <p className="mt-4 font-['DM_Mono'] text-sm tracking-widest text-[#52525B] uppercase">Direct Integration Specialists</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {members.map((member) => (
            <div key={member.id} className="group flex flex-col bg-[#FFFFFF] border border-neutral-200 overflow-hidden hover:border-[#10B981] transition-colors relative">
              <div className="relative w-full aspect-[4/5] bg-neutral-100 overflow-hidden">
                <img
                  src={member.imageKey ? `https://pub-723d911c6a3442c78b2f69b731577d2b.r2.dev/${member.imageKey}` : "/api/placeholder/400/500"}
                  alt={member.name}
                  className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700 scale-105 group-hover:scale-100"
                />
              </div>

              <div className="p-6 flex flex-col flex-1 relative z-10 bg-[#FFFFFF]">
                <h3 className="text-xl font-bold font-['DM_Mono'] text-[#09090B] uppercase tracking-wide">{member.name}</h3>
                <p className="text-xs text-[#52525B] font-['Lora'] tracking-wider mt-1 mb-6 flex-1">{member.role}</p>
                ...
                {member.whatsapp && (
                  <a
                    ...
                    className="mt-auto flex items-center justify-center gap-2 bg-[#25D366] text-[#FAFAFA] py-3 px-4 font-['DM_Mono'] text-xs font-bold uppercase tracking-widest hover:bg-[#1EBE5A] transition-colors shadow-lg shadow-[#25D366]/20"
                  >
                    <WhatsAppIcon /> Let's Connect
                  </a>
                )}
              </div>
            </div>
